...
export function applyCurrentlyPlayingRenderingMethods(CurrentlyPlayingUI) {
    CurrentlyPlayingUI.prototype.render = function render(sessions) {
        ...
        if (visibleItems.length === 0) {
            list.innerHTML = `<div class="analytics-empty-state">${I18n.t('noActiveVideos')}</div>`;
            return;
        }
            ...
            <div class="currently-playing-item">
                <div class="currently-playing-meta">
                    <div class="currently-playing-title-row">
                        <button type="button" class="currently-playing-title currently-playing-title-btn" data-action="focus-tab" data-tab-id="${item.tabId}" title="${I18n.t('openTab')}">${cleanedTitle}</button>
                        <button type="button" class="currently-playing-dismiss" data-action="dismiss-item" data-video-key="${this.escapeHtml(videoKey)}" title="${I18n.t('hideVideo')}">×</button>
                    </div>
                ...
                <div class="currently-playing-controls">
                    <button type="button" class="btn btn-outline btn-sm" data-action="skip-back" data-tab-id="${item.tabId}" data-video-id="${item.video.id}" data-frame-id="${frameId}">${I18n.t('skipBack')}</button>
                    <button type="button" class="btn btn-outline btn-sm currently-playing-control-small" title="${I18n.t('rewind')}" data-action="step-back" data-tab-id="${item.tabId}" data-video-id="${item.video.id}" data-frame-id="${frameId}">⟲</button>
                    <button type="button" class="btn btn-primary btn-sm currently-playing-toggle-btn" data-action="toggle-play" data-tab-id="${item.tabId}" data-video-id="${item.video.id}" data-frame-id="${frameId}">${paused ? I18n.t('play') : I18n.t('pause')}</button>
                    <button type="button" class="btn btn-outline btn-sm currently-playing-control-small" title="${I18n.t('skipAhead')}" data-action="step-forward" data-tab-id="${item.tabId}" data-video-id="${item.video.id}" data-frame-id="${frameId}">⟳</button>
                    <button type="button" class="btn btn-outline btn-sm" data-action="skip-forward" data-tab-id="${item.tabId}" data-video-id="${item.video.id}" data-frame-id="${frameId}">${I18n.t('skipForward')}</button>
                </div>
            </div>
        `;
    };
    ...
    CurrentlyPlayingUI.prototype.cleanTitle = function cleanTitle(title) {
        ...
        if (!raw) return I18n.t('untitledVideo');
        return raw
